...
import { ArrowDown, ArrowLeft, CheckSquare, Square, ChevronRight, Plus } from "lucide-react"
...
interface AddTaskFormInlineProps {
  projectId: string
  currentPath: string[]
}

function AddTaskFormInline({ projectId, currentPath }: AddTaskFormInlineProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (taskName.trim() !== "") {
      addSubtask(projectId, currentPath, taskName)
      setTaskName("")
      setIsFocused(false)
    }
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Escape") {
      setTaskName("")
      setIsFocused(false)
      const inputElement = document.getElementById("add-task-input-inline")
      if (inputElement) {
        inputElement.blur()
      }
    }
  ...
  const placeholderText = currentPath.length === 0 ? "Add task..." : "Add subtask..."

  return (
    <form onSubmit={handleSubmit}>
      <div
        className={cn(
          "flex items-center justify-between p-3 my-1 rounded-md border-2 border-dashed border-muted-foreground/30 transition-all duration-150 h-[46px]",
          isFocused || taskName
...
export function AddTasksView({ isVisible, onClose }: AddTasksViewProps) {
  ...
  const { projects, selectedProjectId, addSubtask, currentFocusTask, refreshFocusTaskPool } = useAppStore()
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {currentProjectId && (
          <div className={currentTasks.length > 0 ? "mt-6" : "mt-2"}>
            <AddTaskFormInline projectId={currentProjectId} currentPath={currentPath} />
          </div>
        )}
      </div>
    </div>
