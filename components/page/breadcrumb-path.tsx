...
export function BreadcrumbPath({ projectName, taskChain }: BreadcrumbPathProps) {
  ...
  // Show only the parent task name (excluding current task)
  const parentTask = taskChain.length > 1 ? taskChain[taskChain.length - 2] : null
  const breadcrumbNames = parentTask ? [parentTask.name] : []
